...
export function getChangedFilePaths(program: ts.BuilderProgram, pathHints?: Array<string>) {
	...
	if (pathHints) {
		for (const hint of pathHints) {
			// search takes lowercase because `buildState.referencedMap` is all lowercase
			// however hints are generated by fs calls, so are in real casing
			search(os.platform() === "win32" ? hint.toLowerCase() : hint);
		}
	} else {
