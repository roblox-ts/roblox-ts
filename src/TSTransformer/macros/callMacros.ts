...
export const CALL_MACROS: MacroList<CallMacro> = {
	assert: {
		// building the truthiness check may re-evaluate and emit statements around the
		// condition; "heap" keeps the arguments stable across them
		effects: "heap",
	...
	typeOf: {
		effects: "none",
		transform: (state, node, expression, args) => luau.call(luau.globals.typeof, args),
	},
	...
	identity: {
		effects: "none",
		transform: (state, node, expression, args) => args[0],
	},
