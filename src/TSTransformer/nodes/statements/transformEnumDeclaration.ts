...
export function transformEnumDeclaration(state: TransformState, prereqs: Prereqs, node: ts.EnumDeclaration) {
	if (
	...
	const inverseId = innerPrereqs.pushToVar(luau.map(), "inverse");
	innerPrereqs.prereq(
	...
	for (const member of node.members) {
		const name = transformPropertyName(state, innerPrereqs, member.name);
		...
		if (typeof value === "string") {
			...
			valueExp = innerPrereqs.pushToVarIfComplex(
				transformExpression(state, innerPrereqs, member.initializer),
				"value",
			);
		}
	...
	const list = luau.list.make<luau.Statement>(
		luau.create(luau.SyntaxKind.DoStatement, { statements: innerPrereqs.statements }),
	);
	if (!isHoisted) {
