	...
	if (index < chain.length - 1) {
		...
		state.prereq(
			lua.create(lua.SyntaxKind.IfStatement, {
				...
				statements: state.statement(() => buildLogicalChainPrereqs(state, chain, conditionId, index + 1)),
...
function mergeInlineExpressions(chain: Array<LogicalChainItem>) {
...
function transformLogicalAnd(state: TransformState, node: ts.BinaryExpression) {
	const chain: Array<LogicalChainItem> = splitBinaryChain(node, ts.SyntaxKind.AmpersandAmpersandToken).map(
	...
	buildLogicalChainPrereqs(state, chain, conditionId);
